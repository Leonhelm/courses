...
Типовые уязвимости - https://cwe.mitre.org/

Common Weakness Enumeration (CWE™) - это разработанный сообществом список распространенных типов программных и аппаратных слабостей, которые имеют последствия для безопасности. "Слабыми местами" являются дефекты, сбои, ошибки или другие ошибки в программной или аппаратной реализации, коде, дизайне или архитектуре, которые, если оставить их без внимания, могут привести к тому, что системы, сети или аппаратное обеспечение станут уязвимыми для атак. Список CWE и связанная с ним классификационная таксономия служат языком, который может быть использован для выявления и описания этих недостатков с точки зрения CWE.

Известные уязвимости - https://cve.mitre.org/

Миссия программы CVE заключается в выявлении, определении и каталогизации публично раскрытых уязвимостей кибербезопасности. На каждую уязвимость в каталоге есть одна запись CVE. Затем обнаруженные уязвимости присваиваются и публикуются организациями со всего мира, сотрудничающими с программой CVE. Партнеры публикуют CVE Records, чтобы сообщать согласованные описания уязвимостей. Специалисты по информационным технологиям и кибербезопасности используют CVE Records для обеспечения обсуждения одного и того же вопроса и координации своих усилий по установлению приоритетов и устранению уязвимостей.
...
Известные инструменты: https://owasp.org/www-community/Component_Analysis

### Работа SCA

Обычно утилиты SCA анализируют файлы проекта, собирает фрагменты информации о зависимостях и строят условные идентификаторы пакетов, такие как CPE — (Common Platform Enumeration) и Package URL (PURL). После получения CPE или PURL утилиты выявляют для CPE/PURL уязвимости из баз данных, таких как NVD, Sonatype OSS Index, NPM Audit API, VulnDB и др. После утилитой предоставляется отчет об анализе компонентов на наличие в их версиях уязвимостей.

Вопрос: Как SCA определяет наличие сторонних компонентов? На основе чего?

1. Менеджеры зависимостей и файлы для сборки
2. Имена файлов
3. Контрольные суммы
...
Популярные источники определения рисков безопасности:

1. [CVE](https://cve.mitre.org/index.html)
2. [NVD](https://nvd.nist.gov/)
3. [VulnDB](https://vulndb.cyberriskanalytics.com/)
4. [Vulners](https://vulners.com/)
5. [ФСТЭК БДУ](https://bdu.fstec.ru/vul) (Just a joke)
...
## Тестирование при помощи фреймворков эксплуатации
...
[**Metasploit Framework**](https://www.metasploit.com/) - удобная платформа для создания и отладки эксплойтов.

**Payload** - "полезная нагрузка", которая выполнится после эксплуатации уязвимости

[**Meterpreter**](https://www.offensive-security.com/metasploit-unleashed/about-meterpreter/) - является продвинутой, динамически расширяемой полезной нагрузкой, которая использует in-memory DLL-injection стейджеры
...
Провести анализ сторонних компонентов ПО для проекта [DVJA](https://github.com/appsecco/dvja)

Этап 1. Установка сборщика Maven
...
Этап 2. Скачиваем исходный код приложения [DVJA](https://github.com/appsecco/dvja)
...
Этап 3. Переходим в директорию с приложением [DVJA](https://github.com/appsecco/dvja) и запускаем сборку с использованием плагина dependency-check-maven
...
Этап 4. Переходим к изучению результатов 

```
firefox ./target/dependency-check-report.html cd
```

Этап 5. Попытаться выявть False Positive или доказать корректно найденную уязвимость пакета

### Упражнение 2

Воспользоваться инстрементарием [GitHub SCA](https://docs.github.com/en/free-pro-team@latest/github/visualizing-repository-data-with-graphs/exploring-the-dependencies-of-a-repository) для сравнения результатов GitHub SCA и Dependency Check в проекте [DVJA](https://github.com/appsecco/dvja).

Этап 1. Зарегистрироваться в сервисе [GitHub](github.com)

Этап 2. Создать ответвление от проекта [DVJA](https://github.com/appsecco/dvja).

Этап 3. Включить в своем репозитории с проектом функции Dependency Graph и Dependency Alerts
...
Эксплуатация недостатков журналирования и мониторинга лежит в основе почти всех крупных взломов.
При проведении атак злоумышленники полагаются на отсутствие контроля и своевременного реагирования на инциденты.
...
- тестирование на проникновение и сканирование инструментами DAST не выдают предупреждений;
- приложение не может определять, реагировать или предупреждать об атаках в реальном или почти реальном времени.

## Как предотвращать такие уязвимости
...
- https://www.anti-malware.ru/compare/SIEM-systems-part2
